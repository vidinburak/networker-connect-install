...
# Önce GitHub API üzerinden asset ID'yi bul, sonra indir
# Bu yöntem private repo asset indirmede daha güvenilir
...
info "Kaynak: ${repo_path} / ${tag} / ${asset_name}"

# GitHub API ile asset URL'ini al
asset_url=$(curl -sfL \
    -H "Authorization: token ${token}" \
    -H "Accept: application/vnd.github.v3+json" \
...
if [ -n "$asset_url" ]; then
    # browser_download_url ile indir
    if curl -fsSL \
        -H "Authorization: token ${token}" \
        --location \
        -o "$tmp_tar" \
...
# Yukarıdaki başarısızsa direkt URL ile dene
